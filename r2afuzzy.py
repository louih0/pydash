...
class R2AFuzzy(IR2A):
    ...
    def output_controller(self, buffer):
        ...
        # factors of the output membership function
        FACTORS = {'N2':0.25,'N1':0.5, 'Z':1, 'P1':1.5, 'P2':2}
        short, close, long, falling, steady, rising = 0,0,0,0,0,0
        
        """
        adopted for the buffering time in order to 
        describe the distance of the current buffering 
        time from a target buffering time T that FDASH
        """
        ...
        """
        Moreover, since for the differential of the buffering
        time input Δti we need to describe the behavior of the rate
        between subsequent buffering times
        """
        ...
        # The fuzzy if-then rules for our controller are the following:
        rules = []
        ...
        # Thus, the linguistic variables of the output are described
        I =  sqrt(rules[8]**2)
    ...
    def calc_throughput(self, throughput, index):
        ...
        # b, d and r = the time when the segment i has been started downloading, and the time
        ...
        #  The throughput of the segment i is estimated at the client as
            ...
            r = (throughput[index])/(throughput[index])
            b = 0.0
            d = 60
            throughput = (b * tau) / (d - r)
        return throughput

        # in relation to the estimated channel throughput over the last period. In particular

        (b + 1) = f * r

        # It should be noted that bi+1 is quantized to the highest
        # available resolution bn that is lower than bi+1.

        if b(n) > b(i):
            """
            the buffer level is estimated to be less than T for the next 60 s, then
            the bit rate remains unchanged
            """
        if b(n) < b(i):
            """
            buffer level for the next 60 s that is larger than T , then the
            bit rate remains unchanged.
            """
        else:
            get.next_bitrate()
